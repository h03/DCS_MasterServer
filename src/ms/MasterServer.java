...
public class MasterServer {
	 ...
	 * 当一个client在6分钟都未向cache server发送数据，则关闭该client与cache server的连接。
	...
	public static void main(String[] args) throws InterruptedException {
		...
		// 开启侦听cache server的线程
		ReceiveFromCache receiveFromCache = new ReceiveFromCache(redisOp);
		receiveFromCache.start(); 
		System.out.println("等待cache连接...");
		
		// 开启侦听user client的询问目标存储节点地址的线程
		RespondToClient respondToClient = new RespondToClient(redisOp);
		respondToClient.start();
		System.out.println("等待user client询问目标cache节点...");
		
		// 开启侦听user client的确认当前目标存储节点状态的线程
		AckToClient ackToClient = new AckToClient(redisOp);
		ackToClient.start();
		System.out.println("等待user client确认当前目标cache节点状态...");
